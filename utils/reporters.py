...
class CustomStateDataReporter(object):
    ...
    def __init__(self, file, reportInterval, step=False, time=False, potentialEnergy=False, kineticEnergy=False, totalEnergy=False, temperature=False, volume=False, density=False,
                 progress=False, remainingTime=False, speed=False, elapsedTime=False, separator=',', systemMass=None, totalSteps=None, append=False, sepenergies=True):
    ...
    def report(self, simulation, state):
        ...
        if not self._hasInitialized:
            ...
            if self._sepenergies:
                self.force_labels = [f"{system.getForce(i).getName()}" for i in range(system.getNumForces())]   
    ...
    def _constructReportValues(self, simulation, state):
        ...
        if self._sepenergies:
            ...
            for i in range(system.getNumForces()):
                state_force = simulation.context.getState(getEnergy=True, groups={i})
                force_energy = state_force.getPotentialEnergy().value_in_unit(unit.kilojoule_per_mole)
                force_energies.append(force_energy)
            for en in force_energies:
                values.append(en)
        return values
